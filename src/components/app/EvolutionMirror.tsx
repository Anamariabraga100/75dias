...
function LockedTeaser({ onLearnMore }: { onLearnMore: () => void }) {
  return (
    <div className="rounded-2xl border border-dashed border-app-border bg-surface p-4">
      <div className="flex gap-3">
        <div className="w-11 h-11 rounded-xl bg-accent-orange/15 flex items-center justify-center shrink-0">
          <Camera size={20} className="text-accent-orange" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-sm text-app-fg">Evolução no espelho</p>
          <p className="text-app-muted text-xs mt-1 leading-relaxed">
            Foto a cada {PHOTO_INTERVAL_DAYS} dias, comparação automática dia 1 vs hoje — exclusivo no
            nível Implacável.
          </p>
          <button
            type="button"
            onClick={onLearnMore}
            className="text-accent-orange text-xs font-semibold mt-2"
          >
            Ver o que inclui →
          </button>
        </div>
...
function ImplacavelTeaserModal({ onClose }: { onClose: () => void }) {
  const navigate = useNavigate()

  return (
    <BottomSheet onClose={onClose}>
      <BottomSheetPanel className="p-5">
        <div className="w-12 h-12 rounded-2xl bg-accent-orange/15 flex items-center justify-center mb-4">
          <Lock size={22} className="text-accent-orange" />
        </div>
        <h3 className="font-bold text-lg text-app-fg mb-2">Recurso do Implacável</h3>
        <p className="text-app-muted text-sm leading-relaxed mb-4">
          No nível mais exigente você registra sua evolução no espelho a cada {PHOTO_INTERVAL_DAYS}{' '}
          dias. O app monta a comparação visual automaticamente — sem álbum solto nem relatórios
          confusos.
        </p>
        <ul className="text-sm text-app-muted space-y-2 mb-5">
          <li>📷 Check-in rápido nos dias 1, 4, 7, 10…</li>
          <li>↔️ Comparação dia 1 vs seu último registro</li>
          <li>🏆 Destaque automático a cada 30 dias</li>
        </ul>
        <button
          type="button"
          onClick={() => {
          ...
          className="w-full btn-primary-themed font-bold py-3 rounded-xl text-sm"
        >
          Escolher nível no Início
        </button>
      </BottomSheetPanel>
    </BottomSheet>
  )
...
export function EvolutionMirror({ displayDay }: EvolutionMirrorProps) {
  ...
  const photoDueToday = isImplacavel && isPhotoDay(displayDay)
  ...
  if (!isImplacavel) {
    return (
      <>
        <section id="evolucao">
          <p className="text-app-muted text-xs uppercase tracking-wide mb-2">Evolução</p>
          <LockedTeaser onLearnMore={() => setShowTeaser(true)} />
        </section>
        {showTeaser && <ImplacavelTeaserModal onClose={() => setShowTeaser(false)} />}
      </>
    )
  }
